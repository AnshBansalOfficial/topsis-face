...
while True:
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            # Define new regions for the facial mask
            ...
            # Draw smaller circles to form the mask (reducing visual size)
            for region in mask_regions:
                for point in region:
                    cv2.circle(frame, point, 1, (0, 255, 255), -1)  # Draw small circles for landmarks

                # # Draw lines between points in the region
                # for i in range(len(region) - 1):
                #     cv2.line(frame, region[i], region[i + 1], (0, 255, 255), 2)  # Draw lines
                ...
                #     cv2.line(frame, region[-1], region[0], (0, 255, 0), 2)

    # Display the frame
    cv2.imshow("Detailed Face Mask", frame)
